...
import { Layout, Typography, Button, Divider, Space, InputNumber, Table, Collapse, Select, Card, theme, Upload, message, Modal } from 'antd';
...
function Ahp() {
  ...
  try {
    ...
    // Chekced matrix is immediately adopted 
    setCriteria(c);
    setMatrix(M);
  ...
  return false; // Upload의 네트워크 업로드 막기
};

  ////// Manual input for AHP //////
    ...
    return (
        ...
        <Sider width={420} style={{ background: token.colorBgContainer, padding: token.paddingLG, borderRight: `1px solid ${token.colorSplit}` }}>
            ...
            <div style={{ margin: '8px 0 24px' }}>
              ...
              >
                <Button icon={<UploadOutlined />}>Import AHP CSV/XLSX</Button>
              </Upload>
            </div>
        ...
        {step === "define-alternatives" && (
            <Layout>
            <Sider width={420} style={{ background: token.colorBgContainer, padding: token.paddingLG }}>
            ...
            <Content style={{ padding: token.paddingLG, background: token.colorBgLayout }}> 
                ...
                <>
                    ...
                    <>
                        <Divider />
                        <Typography.Title level={4}>Compare Alternative criteria</Typography.Title>
                        <Collapse accordion>
                        {criteria.map((crit, index) => (
